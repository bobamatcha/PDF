...
var PerformanceMonitor = class _PerformanceMonitor {
  constructor() {
    ...
    this.loadingIndicators = /* @__PURE__ */ new Map();
  ...
  logMetrics() {
    ...
    if (metrics.timeToInteractive !== void 0) {
...
var perf = PerformanceMonitor.getInstance();
...
function withTimingSync(name, fn) {
  perf.start(name);
  try {
    return fn();
  } finally {
...
async function withLoading(message, fn, containerId) {
...
function passesFilter(namespace) {
...
function enableLogging(level = "info", filter) {
  config.enabled = true;
  config.minLevel = level;
  if (filter !== void 0) {
...
var log = createLogger("PdfLoader");
...
var log2 = createLogger("SyncManager");
...
var SyncManager = class {
  constructor(config2) {
    ...
    this.config = {
      syncEndpoint: config2.syncEndpoint,
      minBackoffMs: config2.minBackoffMs ?? 1e3,
      maxBackoffMs: config2.maxBackoffMs ?? 3e4,
      retryIntervalMs: config2.retryIntervalMs ?? 3e4,
      maxRetries: config2.maxRetries ?? 10
    };
  ...
  start() {
    ...
    this.isStarted = true;
    log2.info("Starting sync manager");
  ...
  stop() {
    ...
    this.isStarted = false;
    log2.info("Stopping sync manager");
  ...
  async syncNow() {
    if (isExplicitOfflineMode()) {
      log2.debug("Skipping sync - explicit offline mode");
      return;
    }
    if (!navigator.onLine) {
      log2.debug("Skipping sync - offline");
      return;
    }
    if (this.isSyncing) {
      log2.debug("Skipping sync - already in progress");
      return;
    }
    const queue = getOfflineQueue();
    if (queue.length === 0) {
      log2.debug("Nothing to sync");
      return;
    }
    this.isSyncing = true;
    ...
    log2.info(`Starting sync of ${queue.length} items`);
    let syncedCount = 0;
    ...
    log2.info(`Sync completed: ${syncedCount}/${queue.length} items`);
  }
  /**
  ...
  setOfflineMode(enabled) {
    ...
    if (enabled) {
      ...
      log2.info("Offline mode enabled");
    } else {
      localStorage.removeItem(OFFLINE_MODE_KEY);
      log2.info("Offline mode disabled");
      if (navigator.onLine) {
        this.syncNow();
      }
    }
   ...
   */
  notifyNewSignature() {
    log2.debug("New signature saved, checking for sync");
  ...
  startPeriodicRetry() {
    ...
    this.periodicRetryId = setInterval(() => {
      if (navigator.onLine && !isExplicitOfflineMode() && !this.isSyncing) {
        const queue = getOfflineQueue();
        if (queue.length > 0) {
          log2.debug("Periodic retry triggered");
          this.syncNow();
        }
      }
  ...
  async syncItem(item) {
    ...
    if (attemptCount > this.config.maxRetries) {
      log2.warn(`Max retries exceeded for ${errorKey}, skipping`);
      return false;
    }
    try {
      ...
      if (response.ok) {
        ...
        log2.debug(`Successfully synced ${errorKey}`);
        return true;
      }
      ...
      log2.error(`Failed to sync ${errorKey}:`, errorMessage);
  ...
  async handleConflict(item, serverData) {
    log2.debug(`Handling conflict for ${item.sessionId}`);
    if (serverData.serverTimestamp && serverData.serverTimestamp > item.timestamp) {
      log2.debug("Server has newer data, preferring server");
      removeFromOfflineQueue(item.sessionId, item.recipientId);
      return;
    }
    if (serverData.signatures && item.signatures) {
      log2.debug("Merging local signatures with server");
      ...
      if (retryResponse.ok) {
        removeFromOfflineQueue(item.sessionId, item.recipientId);
        log2.debug("Conflict resolved with merge");
      }
    } else {
      removeFromOfflineQueue(item.sessionId, item.recipientId);
    }
...
function getSyncManager(config2) {
  if (!syncManagerInstance) {
    if (!config2) {
      throw new Error("SyncManager not initialized. Call getSyncManager with config first.");
    }
    syncManagerInstance = new SyncManager(config2);
  }
  return syncManagerInstance;
}
function initSyncManager(config2) {
  const manager = getSyncManager(config2);
  manager.start();
  return manager;
}

// src/ts/sign-pdf-bridge.ts
var log3 = createLogger("DocSign");
  ...
  return {
    async loadPdf(data) {
      try {
        ...
        log3.info("PDF loaded:", pageCount, "pages");
        return {
          numPages: pageCount,
          success: true
        };
        ...
        const errorMessage = err instanceof Error ? err.message : String(err);
        log3.error("Failed to load PDF:", errorMessage);
        pageCount = 0;
        return {
          numPages: 0,
          success: false,
    ...
    async renderAllPages(config2) {
      const { container, scale = 1.5, pageWrapperClass = "pdf-page-wrapper" } = config2;
      const results = [];
      if (!PdfPreviewBridge.currentDoc) {
        log3.error("No document loaded");
        return results;
      }
      container.innerHTML = "";
      ...
      log3.debug("Rendered", results.length, "pages");
      return results;
    },
    async renderPage(pageNum, canvas, scale = 1.5) {
      try {
        ...
        const errorMessage = err instanceof Error ? err.message : String(err);
        log3.error(`Failed to render page ${pageNum}:`, errorMessage);
        return {
          pageNum,
          dimensions: {
            width: 0,
    ...
    cleanup() {
      PdfPreviewBridge.cleanup();
      pageCount = 0;
      log3.debug("Cleaned up PDF resources");
    },
    isDocumentLoaded() {
...
function initDocSignNamespace() {
  ...
  log3.info("PDF bridge, error handling, and sync manager initialized on window.DocSign");
}

// src/ts/crypto-utils.ts
var log4 = createLogger("CryptoUtils");
var SEED_STORAGE_KEY = "docsign_crypto_seed";
var KEY_ALGORITHM = "AES-GCM";
var KEY_LENGTH = 256;
var IV_LENGTH = 12;
var cachedKey = null;
function getOrCreateSeed() {
  ...
  if (!seed) {
    ...
    log4.debug("Generated new device seed");
  }
  return seed;
}
async function deriveKey() {
  ...
  const salt = new TextEncoder().encode("docsign-indexeddb-encryption-v1");
  ...
  log4.debug("Derived encryption key");
  return cachedKey;
}
...
async function decryptBytes(encrypted) {
  const key = await deriveKey();
...
function isEncrypted(data) {
  ...
  const obj = data;
  return typeof obj.ciphertext === "string" && typeof obj.iv === "string" && obj.version === 1;
}
...
var log5 = createLogger("LocalSessionManager");
...
function openDatabase() {
  return new Promise((resolve, reject) => {
    ...
    request.onerror = () => {
      log5.error("Failed to open database:", request.error);
    ...
    request.onupgradeneeded = (event) => {
      ...
      log5.info("Database schema created/upgraded");
    };
  });
}
...
var LocalSessionManager = class {
  ...
  static async getSession(sessionId, _recipientId) {
    try {
      ...
      if (!session) {
        log5.debug("Session not found locally:", sessionId);
        return void 0;
      }
      if (session.expiresAt) {
        const expiresAt = new Date(session.expiresAt).getTime();
        if (Date.now() > expiresAt) {
          log5.debug("Session expired:", sessionId);
          session.status = "expired";
          return session;
        }
      }
      log5.debug("Session found locally:", sessionId);
      return session;
    } catch (err) {
      log5.error("Error getting session:", err);
      return void 0;
    }
  }
  /**
  ...
  static async saveSession(session) {
    try {
      ...
      log5.debug("Session saved:", session.sessionId);
    } catch (err) {
      log5.error("Error saving session:", err);
      throw err;
    }
  }
  /**
  ...
  static async cacheSession(serverResponse) {
    try {
      ...
      log5.debug("Server session cached:", session.sessionId);
    } catch (err) {
      log5.error("Error caching server session:", err);
    }
  }
  /**
    ...
    try {
      ...
      if (isEncryptionAvailable()) {
        try {
          ...
          log5.debug("PDF data encrypted");
        } catch (encryptErr) {
          log5.warn("Encryption failed, storing unencrypted:", encryptErr);
      ...
      log5.debug("PDF data cached for session:", sessionId);
    } catch (err) {
      log5.error("Error caching PDF data:", err);
    }
  }
  /**
  ...
  static async getCachedPdfData(sessionId) {
    try {
      const cached = await getFromStore(STORES.PDF_CACHE, sessionId);
      ...
      if (cached.isEncrypted && isEncrypted(cached.pdfData)) {
        try {
          ...
          log5.error("Failed to decrypt PDF data:", decryptErr);
          return void 0;
        }
      }
      return cached.pdfData;
    } catch (err) {
      log5.error("Error getting cached PDF:", err);
      return void 0;
    }
  }
  /**
      ...
      if (session) {
        ...
        if (isEncryptionAvailable()) {
          try {
            ...
            log5.debug("Signatures encrypted");
          } catch (encryptErr) {
            log5.warn("Signature encryption failed:", encryptErr);
            storedSignatures = mergedSignatures;
          }
        } else {
          storedSignatures = mergedSignatures;
        }
        session.signatures = storedSignatures;
        session.status = "in_progress";
        if (!session.metadata) {
          session.metadata = {};
        }
        session.metadata.signaturesEncrypted = isSignaturesEncrypted;
        await this.saveSession(session);
        log5.debug("Signatures saved for session:", sessionId);
      } else {
        log5.warn("Cannot save signatures - session not found:", sessionId);
      }
    } catch (err) {
      log5.error("Error saving signatures:", err);
      throw err;
    }
  }
  /**
   ...
   */
  static async getDecryptedSignatures(sessionId) {
    try {
      const session = await this.getSession(sessionId);
      if (!session?.signatures) return void 0;
      const isSignaturesEncrypted = session.metadata?.signaturesEncrypted;
      if (isSignaturesEncrypted && isEncrypted(session.signatures)) {
        try {
          const decrypted = await decryptString(session.signatures);
          return JSON.parse(decrypted);
        } catch (decryptErr) {
          log5.error("Failed to decrypt signatures:", decryptErr);
          return void 0;
        }
      }
      return session.signatures;
    } catch (err) {
      log5.error("Error getting signatures:", err);
      return void 0;
    }
  }
  /**
  ...
  static async completeSession(sessionId) {
    try {
      ...
      if (session) {
        ...
        await this.saveSession(session);
        log5.debug("Session completed:", sessionId);
      }
    } catch (err) {
      log5.error("Error completing session:", err);
      throw err;
    }
  }
  /**
  ...
  static async queueForSync(submission) {
    try {
      ...
      log5.debug("Submission queued for sync:", submission.sessionId);
    } catch (err) {
      log5.error("Error queueing submission:", err);
      throw err;
    }
  }
  /**
  ...
  static async getQueuedSubmissions() {
    try {
      ...
      log5.error("Error getting queued submissions:", err);
      return [];
    }
  }
  /**
  ...
  static async removeFromQueue(sessionId, recipientId) {
    try {
      ...
      log5.debug("Removed from queue:", sessionId, recipientId);
    } catch (err) {
      log5.error("Error removing from queue:", err);
    }
  }
  /**
  ...
  static async deleteSession(sessionId) {
    try {
      ...
      log5.debug("Session deleted:", sessionId);
    } catch (err) {
      log5.error("Error deleting session:", err);
    }
  }
  /**
  ...
  static async getSessionsForRecipient(recipientId) {
    try {
      ...
      log5.error("Error getting sessions for recipient:", err);
      return [];
    }
  }
  /**
  ...
  static async clearAll() {
    try {
      ...
      log5.info("All local data cleared");
    } catch (err) {
      log5.error("Error clearing all data:", err);
    }
  }
  /**
...
function initLocalSessionNamespace() {
  ...
  log5.info("Session management initialized on window.DocSign");
}
...
var shownJokeIndices = /* @__PURE__ */ new Set();
...
var log7 = createLogger("LoadingOverlay");
var DEFAULT_PUNCHLINE_DELAY = 1500;
var MIN_DISPLAY_TIME = 800;
var overlayElement = null;
var currentJoke = null;
var showStartTime = 0;
var punchlineTimeout = null;
...
function initLoadingOverlay() {
  ...
  log7.debug("Loading overlay initialized");
}
function showLoadingOverlay(statusText = "Loading...") {
  ...
  log7.debug("Showing loading overlay");
}
function hideLoadingOverlay() {
  ...
  setTimeout(() => {
    ...
    currentJoke = null;
    log7.debug("Hiding loading overlay");
  }, remaining);
}
...
async function withLoadingOverlay(fetchFn, statusText = "Loading...") {
...
function initLoadingNamespace() {
  if (typeof window !== "undefined" && window.DocSign) {
    ...
    log7.debug("Loading overlay added to window.DocSign");
  }
}

// src/ts/main.ts
var log8 = createLogger("DocSign");
...
function init() {
  ...
  log8.info("DocSign TypeScript initialized");
  log8.debug("PDF Preview Bridge available:", typeof PdfPreviewBridge !== "undefined");
  log8.debug("DocSign namespace available:", typeof window.DocSign !== "undefined");
  log8.debug("LocalSessionManager available:", typeof LocalSessionManager !== "undefined");
  log8.debug("SyncManager available:", typeof SyncManager !== "undefined");
  log8.debug("TypedSignature available:", typeof TypedSignature !== "undefined");
  log8.debug("MobileSignatureModal available:", typeof MobileSignatureModal !== "undefined");
  log8.debug("SignatureCapture available:", typeof SignatureCapture !== "undefined");
  log8.debug("SignatureCaptureModal available:", typeof SignatureCaptureModal !== "undefined");
  log8.debug("Auth module available:", typeof isAuthenticated !== "undefined");
  log8.debug("User authenticated:", isAuthenticated());
  log8.debug("Loading overlay available:", typeof showLoadingOverlay !== "undefined");
}
