...
/**
 * Initialize LocalSessionManager on window.AgentPDF namespace
 */
export function initLocalSessionNamespace(): void {
  // Ensure AgentPDF namespace exists
  const win = window as { AgentPDF?: Record<string, unknown> };
  if (!win.AgentPDF) {
    win.AgentPDF = {};
  }

  const agentPDF = win.AgentPDF;

  // Add LocalSessionManager class
  agentPDF.LocalSessionManager = LocalSessionManager;

  // Add singleton instance
  agentPDF.localSessionManager = localSessionManager;

  // Add convenience functions bound to the singleton
  agentPDF.createSession = localSessionManager.createSession.bind(localSessionManager);
  agentPDF.getSession = localSessionManager.getSession.bind(localSessionManager);
  agentPDF.updateSessionStatus = localSessionManager.updateSessionStatus.bind(localSessionManager);
  agentPDF.recordSignature = localSessionManager.recordSignature.bind(localSessionManager);
  agentPDF.getSignedDocument = localSessionManager.getSignedDocument.bind(localSessionManager);
  agentPDF.deleteSession = localSessionManager.deleteSession.bind(localSessionManager);
  agentPDF.listSessions = localSessionManager.listSessions.bind(localSessionManager);

  log.info('Session management initialized on window.AgentPDF');
}
