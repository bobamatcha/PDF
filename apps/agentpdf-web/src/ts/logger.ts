/**
 * Structured Logging Utility for AgentPDF
 *
...
function initConfig(): void {
  if (typeof window === 'undefined') {
    // Non-browser environment (tests) - enable by default
    config.enabled = true;
  ...
  try {
    ...
    if (!config.enabled && typeof localStorage !== 'undefined') {
      const stored = localStorage.getItem('agentpdf:log');
      if (stored === '1' || stored === 'true') {
        config.enabled = true;
      }
      const storedLevel = localStorage.getItem('agentpdf:logLevel');
      if (storedLevel && storedLevel in LOG_LEVELS) {
        config.minLevel = storedLevel as LogLevel;
      }
      const storedFilter = localStorage.getItem('agentpdf:logFilter');
      if (storedFilter) {
        config.filter = storedFilter;
      }
    }

    // Default: enable on localhost or file:// protocol
    if (!config.enabled) {
...
export function enableLogging(level: LogLevel = 'info', filter?: string): void {
  ...
  try {
    if (typeof localStorage !== 'undefined') {
      localStorage.setItem('agentpdf:log', '1');
      localStorage.setItem('agentpdf:logLevel', level);
      if (filter) {
        localStorage.setItem('agentpdf:logFilter', filter);
      }
    }
  } catch {
    // Ignore errors
...
export function disableLogging(): void {
  ...
  try {
    if (typeof localStorage !== 'undefined') {
      localStorage.removeItem('agentpdf:log');
      localStorage.removeItem('agentpdf:logLevel');
      localStorage.removeItem('agentpdf:logFilter');
    }
  } catch {
    // Ignore errors
  }
...
export const loggers = {
  AgentPDF: createLogger('AgentPDF'),
